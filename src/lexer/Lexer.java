...
public class Lexer {
	...
	// necessário para diferenciar A B de A.B ou A = B, etc
	public boolean isNextTokenIdent()
	{
		...
		while (  (ch = input[tempTokenPos]) == ' ' || ch == '\r' ||
								 ch == '\t' || ch == '\n')
		{
			tempTokenPos++;
				}
		...
		while(true)
		{
			...
			else if ( input[tempTokenPos] == '/' && input[tempTokenPos + 1] == '/' )
			{
				// comment found
				while ( input[tempTokenPos] != '\0'&& input[tempTokenPos] != '\n' )
					tempTokenPos++;
			...
			else if ( input[tempTokenPos] == '/' && input[tempTokenPos + 1] == '*' )
			{
				...
				while ( (ch = input[tempTokenPos]) != '\0' && (ch != '*' || input[tempTokenPos + 1] != '/') ) {
					tempTokenPos++;
				}
			...
			else
			{
				if ( Character.isLetter( ch ) )
				{
					...
					while ( Character.isLetter( ch = input[tempTokenPos] ) ||
									Character.isDigit(ch) ||
									ch == '_' ) {
							ident.append(input[tempTokenPos]);
							tempTokenPos++;
					}
					String tempStringValue = ident.toString();
						// if identStr is in the list of keywords, it is a keyword !
					Symbol value = keywordsTable.get(tempStringValue);
